...
class DaeTrainer(object):
    ...
    def get_learned_representation(self):
        self.best_model.eval()
        with torch.no_grad():
            learned_feats = []
            for batch_idx, x_batch in enumerate(self.test_loader):
                x_batch_noise = self.tensor_swapNoise(x_batch)
                x_batch_noise, x_batch = x_batch_noise.cuda(), x_batch.cuda()
    #             x_batch = x_batch.type(torch.LongTensor).cuda()
                hs = []
                h = x_batch_noise
                for module in list(self.best_model.children())[:-1]:
                    h = module(h)
                    hs.append(h.view(h.size(0), -1).cpu().numpy())
                learned_feats.append(np.concatenate(hs, axis=1))
            learned_feats = np.concatenate(learned_feats, axis=0)
        self.logger.info(f'learned feature shape: {learned_feats.shape}')
        return learned_feats

    def train_nn(self):
        self.min_val_loss = np.inf
        # if self.cycle:
        #     step_size = 2000
        #     scheduler = CyclicLR(optimizer, base_lr=self.base_lr, max_lr=self.max_lr,
    ...
    #             x_batch = x_batch.type(torch.LongTensor).cuda()
                ...
                output = self.net(x_batch_noise)
                # if self.cycle:
                # print('cycle_LR')
                # scheduler.batch_step()
    #             loss = criterion(output.squeeze(), y_batch)
                loss = self.criterion(output.squeeze(), x_batch)
                loss.backward()
                t_loss += loss.detach().cpu()
                self.optimizer.step()
            t_loss = t_loss.item()/(batch_idx+1)
            epoch_message = "epoch [{}/{}] elapsed:{:.0f}s  {} Train:{:.6f}  ".format(
                epoch+1, self.max_epoch, time.time()-start, self.loss_name, t_loss)
            self.net.eval()
            with torch.no_grad():
        #         loss = 0
        #         for batch_idx, x_batch in enumerate(self.train_loader):
        ...
        #             cur_loss = self.criterion(output.squeeze(), x_batch)
        #             loss += cur_loss
        #         t_loss = loss.item()/(batch_idx+1)
                # train_message = epoch_message+'{} Train : {:.6f} \t'.format(self.loss_name,
                                                                            # t_loss)
                loss = 0
        ...
        #             cur_loss = criterion(output, y_batch)
                ...
                # self.logger.info(train_message+'Valid: {:.6f} \t'.format(loss))
                self.logger.info(epoch_message+'Valid: {:.6f}  '.format(loss))
            ...
            check_stop = self.check_learning(epoch, loss)
            if check_stop:
                break
    ...
    def kfold_nn(self):
        ...
        self.train_df = self.train_df.append(self.test_df)
        self.test_loader = DataLoader(torch.FloatTensor(
            self.train_df.loc[:, self.feats].values), batch_size=self.batchsize, shuffle=False)
        learned_feats = self.get_learned_representation()
    ...
    def tensor_swapNoise(self, tensor):
        n, f = tensor.size()
        swap_n = round(n*self.swap_p)
        swap_idx = torch.cat([torch.randperm(n).unsqueeze(1)
                              for k in range(f)], dim=1).type(torch.LongTensor)
        swap_idx = swap_idx[:swap_n*2]

        swapped_tensor = torch.zeros_like(tensor)
        swapped_tensor.data = tensor.clone()
        for k in range(f):
            swapped_tensor[swap_idx[:swap_n, k],
                           k] = tensor[swap_idx[swap_n:, k], k]
